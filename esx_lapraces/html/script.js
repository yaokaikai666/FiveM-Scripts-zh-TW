    ...
    if (type == "creator") {
        if (data.active) {
            if (!CreatorActive) {
                ...
                $("#editor-keys-tiredistance").html('<span style="color: rgb(0, 201, 0);">ARROW LEFT  </span> / <span style="color: rgb(255, 43, 43);">ARROW RIGHT </span> - Tire Distance ['+data.data.TireDistance+'.0]');
        ...
        if (data.active) {
            if (!RaceActive) {
                ...
                $("#race-time").html('Current Lap: ' + secondsTimeSpanToHMS(data.data.Time));
